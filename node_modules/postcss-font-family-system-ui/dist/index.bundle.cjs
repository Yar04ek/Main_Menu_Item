...
function computePolyfillFlagFromStatus(statusString) {
  const flags = [];
  const statusArr = Array.from(statusString.split(" "));
...
function generatePolyfillFlags(stats, supportedBrowsers) {
  ...
  for (const browserAndVersion of supportedBrowsers) {
    ...
    shouldPolyfillFlags.forEach(flag => {
      polyfillFlags[flag] = true;
    });
  }
...
function getSystemUiFamily(supportedBrowsers, preserve) {
  const {
    stats
  } = caniuse.feature(caniuse.features["font-family-system-ui"]);
  const polyfillFlags = generatePolyfillFlags(stats, supportedBrowsers);
  const polyfill = [[APPLE_SYSTEM, ["-apple-system" // macOS, Safari >= 9.2 < 11, Firefox >= 43
  ]], [BLINK_MAC_SYSTEM_FONT, ["BlinkMacSystemFont" // macOS, Chrome < 56
  ]], [NO, ["Segoe UI", // Windows >= Vista
  "Roboto", // Android >= 4
  "Noto Sans", // Plasma >= 5.5
  "Ubuntu", // Ubuntu >= 10.10
  "Cantarell", // GNOME >= 3
  "Helvetica Neue"]]];
  const result = [].concat.apply([], polyfill.filter(([flag]) => polyfillFlags[flag]).map(v => v[1]));

  if (!result.length || preserve) {
    result.unshift("system-ui");
  }
...
const creator = opts => {
  ...
  const whitespace = "[\\f\\n\\r\\x09\\x20]";
  const systemUiMatch = new RegExp(`(^|,|${whitespace}+)(?:system-ui${whitespace}*)(?:,${whitespace}*(?:${systemUiFamily.join("|")})${whitespace}*)?(,|$)`, "i"); // system-ui fallback replacement
